...
void loop() 
{
  ...
  xOmega=-xGyro*0.01; //-xGyro*M5.IMU.gRes //M5.IMU.gRes=0.01 https://docs.m5stack.switch-science.com/en/arduino/m5stickc/sh200q_m5stickc degrees per second. TODO: check it

  roll = -360.0/6.28*atan2(-xAcc,zAcc);
  pitch = 360.0/6.28*atan2(yAcc,zAcc);
  pitch_filtered= 0.993*(pitch_filtered+xGyro*h)+(1.0-0.993)*(pitch); //low pass filter + complementary filter (handmade). TODO: double check it!
  ...
  if (abs(error)>30) //if the error is big, do nothing since the segway has fallen
  {
    u=0;
  }
  if (u>127) u=127; //check for saturation
  if (u<-127) u=-127;

  Wire.beginTransmission(0x38); //send data to motors
  Wire.write(0); //channel left
  Wire.write((int)(-u));
  Wire.endTransmission();
  Wire.beginTransmission(0x38);
  Wire.write(1); //channel right
  Wire.write((int)(u));
  Wire.endTransmission();
